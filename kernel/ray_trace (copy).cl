...
bool traceOctree(__global Node* voxels, float3 origin, float3 direction, int* iterations) {
        ...
        while (true) {
            *iterations += 1;
            ...
            if (exitNode) {
                ...
                if (node.size == root.size) return false;
            ...
            t = max(t0Child.x, max(t0Child.y, t0Child.z));
        }
...
__kernel void ray_trace(__write_only image2d_t pixels, float16 invMatrix, float3 eye, float time, float3 lightPosition, __global Node* voxels) {
    ...
    if (traceOctree(voxels, eye, direction, &iterations)) {

    } else {
        color.xyz *= (float)iterations / 10.0f;
    }
